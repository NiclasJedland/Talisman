		...
		static void Main(string[] args)
		{
			...
			using(StreamReader reader = new StreamReader(Directory.GetCurrentDirectory() + @"\talisman.json"))
			{
				...
				foreach(var expansion in monsters)
				{
					...
					{
						foreach(var stat in mob)
						{
							if(stat.Name.ToString().ToLower().Contains("goblin"))
							{
								stats.countGoblins++;
								gobLst.Add(stat.Name.ToString());
							}

							if(stat.Description != "")
								dic.Add(stat.Name.ToString(), stat.Description.ToString());

							if(stat.strength == "?" || stat.craft == "?")
							{
								stats.numberOfQuestionmarkMonsters++;
								continue;
							}
							int str = int.Parse(stat.strength.ToString());
							int craft = int.Parse(stat.craft.ToString());

							CountType(stat.EnemyType.ToString());
